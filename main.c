...
struct books* readbooks() {
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (token == NULL) goto cleanup;
        strcpy(p->ISBN, token);
        p->ISBN[MAX_LENGTH - 1] = '\0';


        // 读取书名
        token = strtok_s(NULL, " ", &nextToken);
        if (token == NULL) goto cleanup;
        strncpy(p->name, token, MAX_LENGTH - 1);
        p->name[MAX_LENGTH - 1] = '\0';

        // 读取作者
        token = strtok_s(NULL, " ", &nextToken);
        if (token == NULL) goto cleanup;
        strncpy(p->auther, token, MAX_LENGTH - 1);
        p->auther[MAX_LENGTH - 1] = '\0';

        // 读取出版社
        token = strtok_s(NULL, "\n", &nextToken);
        if (token == NULL) goto cleanup;
        strncpy(p->publish, token, MAX_LENGTH - 1);
        p->publish[MAX_LENGTH - 1] = '\0';
